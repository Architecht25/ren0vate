...
|---|---|---|---|---|
| Wallonie — nouveau régime prêt bonifié (calcul) | ✅ **Réel et fonctionnel**, pas une façade. `WallonieRegimeRouter` (`app/services/regions/wallonie/wallonie_regime_router.rb`), `PretReduction::EligibilityService` (critère PEB D/E/F bloquant), `TrancheService` (seuils 28 900/41 100/67 100/122 800€, taux 50/40/15/0%), `CalculatorService` (plafond 75 000€, calcul réel sur revenu utilisateur). Vue `_pret_reduction_card.html.erb` affiche un avertissement "enveloppe non fixée avant conclave rentrée 2026" — honnête. Tests régions : 43 runs/86 assertions, 0 failure. | — | — | — |
...
| Wallonie — validation métier du critère PEB (label G) | Le code exclut PEB hors `D/E/F` mais un commentaire du dev admet que le texte réglementaire source ne tranche pas explicitement le cas du label G. Point d'expertise terrain, pas de code. | 🟠 Important | Faible (relecture experte) | Expertise primes de Robin |
...
| 🔴 Test d'intégration onboarding en échec | `bin/rails test` → 89 runs/208 assertions/**1 failure**. `OnboardingSmokeTest#test_tunnel_propriétaire` attend un redirect vers `/dashboard`, obtient `/projects/8?tab=preparation`. À trancher : régression réelle du tunnel ou test obsolète face à un comportement changé volontairement — pas tranché dans cet audit. | 🔴 Bloquant (avant tout test pro) | Faible (triage + fix) | — |
...
| 🟠 Bug — `Subscription#sync_with_stripe!` lit une API Stripe obsolète | `app/models/subscription.rb:64-69` lit `stripe_subscription.current_period_start/end` à la racine, alors que l'API `2026-04-22.dahlia` a déplacé ces champs dans `items.data[0].current_period` (déjà documenté comme piège connu dans CLAUDE.md, et déjà corrigé dans les handlers webhook `webhooks_controller.rb:106-109,141-144` — mais **pas** dans ce sync model). `rescue Stripe::StripeError` avale l'erreur silencieusement : les dates de période ne se rafraîchissent probablement pas sur annulation/sync manuel. | 🟠 Important | Faible (aligner sur le pattern déjà utilisé dans les webhooks) | — |
...
| Memory quota Heroku (R14) | Signalé en mai comme risque de montée en charge (Solid Queue in-process Puma, 513-544MB sur quota 512MB standard-1x). **Non vérifiable depuis le repo** — à recontrôler sur Heroku avant l'envoi à 15 000 contacts (pic de trafic possible). | 🟠 Important | Faible (upgrade dyno ou déplacer Solid Queue) | Accès Heroku |
...
5. **PEB label G en Wallonie.** Le code exclut actuellement tout ce qui n'est pas D/E/F du nouveau régime prêt bonifié — confirmation d'expert nécessaire (le texte réglementaire source ne semble pas trancher explicitement ce cas).
