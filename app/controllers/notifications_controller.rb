class NotificationsController < ApplicationController
  ...
  def create_admin
    ...
    when 'primes_services'
      target_users = User.where(primes_services_client: true)
    when 'non_primes_services'
      target_users = User.where(primes_services_client: false)
  ...
  def get_related_items(notification)
    ...
    if notification.property
      ...
      items[:property_documents] = Document.for_property_and_its_projects(notification.property).order(created_at: :desc).limit(3)
    end
